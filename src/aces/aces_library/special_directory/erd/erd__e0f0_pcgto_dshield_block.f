...
C  in the file COPYRIGHT.
         SUBROUTINE  ERD__E0F0_PCGTO_DSHIELD_BLOCK
     +
     +                    ( NBATCH,NINT2D,
...
C                                    (x = X,Y,Z)
C
C                    INT2Dx       =  will hold all current 2D^(1) integrals
C                                    for each cartesian component
C                                    (x = X,Y,Z)
C
C                  Output:
...
C
         ...
         INTEGER     CASE2D_0,CASE2D_1,CASEAT
         ...
         DOUBLE PRECISION  INT2DX1  (1:NINT2D)
         DOUBLE PRECISION  INT2DY1  (1:NINT2D)
         DOUBLE PRECISION  INT2DZ1  (1:NINT2D)
...
C         Write(6,*) (PRIMA(ij), ij=1,mij)
C         Write(6,*) (PRIMB(ij), ij=1,mij)
C         Write(6,*) (PRIMC(ij), ij=1,mkl)
C         Write(6,*) (PRIMD(ij), ij=1,mkl)

         CASE2D_0 = 3 * MIN0 (2,SHELLQ) + MIN0 (2,SHELLP) + 1
         CASE2D_1 = 3 * MIN0 (2,SHELLQ+1) + MIN0 (2,SHELLP) + 1
C
C
C         Write(6,*) "ATOMIC, CASEAT,CASE2D", ATOMIC, CASEAT, CASE2D_0,
C     &               CASE2D_1
...
C         Write(6,*) "ATOMAB ATOMCD,NIJBEG,NIJEND,NKLBEG,NKLEND"
C         Write(6,*) ATOMAB, ATOMCD, NIJBEG,NIJEND,NKLBEG,NKLEND
...
C
C
C         Write(*,*) "ERD__RYS_ROOTS_WEIGHTS"
     ...
     +
C
C         Write(*,*) "END ERD__RYS_ROOTS_WEIGHTS"
C
...
CSSS             write(*,*) "CASE2D", CASE2D_0,CASE2D_1
     ...
     +
     +
CSSS             Write(*,*) "EP0-1 batches"
CSSS             Write(*,"(3(1x,F15.10))") (BATCH (I), I=1, NBATCH)
     ...
     +
     +
CSSS             Write(*,*) "E0Q-1 batches"
CSSS             Write(*,"(3(1x,F15.10))") (BATCH (I), I=1, NBATCH)
 
             ELSE

                 CALL    ERD__JNT2D_TO_E0F0
     +
     +                        ( SHELLA,SHELLP,SHELLC,SHELLQ,
     +                          NGQP,MIJKL,MGQIJKL,
     +                          NXYZET,NXYZFT,NXYZP,NXYZQ,
     ...
     +
     +
CSSS             Write(*,*) "EPQ-1 batches"
CSSS             Write(*,"(3(1x,F15.10))") (BATCH (I), I=1, NBATCH)

             END IF

         ELSE
CSSS             write(*,*) "Enter  ERD__2D_ATOM_DSHIELD_PQ_INTEGRALS"
             CALL    ERD__2D_ATOM_DSHIELD_PQ_INTEGRALS
     +
     +                    ( MIJ,MKL,MIJKL,NGQP,MGQIJKL,
     ...
     +
     +
CSSS          write(*,*) " End ERD__2D_ATOM_DSHIELD_PQ_INTEGRALS"
CSSS          WRITE(*,*) SHELLP, SHELLQ

             IF (SHELLQ.EQ.0) THEN

CSSS                 write(*,*) "Enter  ERD__ATOM_INT2D_TO_E000"
                 CALL    ERD__ATOM_JNT2DDS_TO_E000
     +
     +                        ( SHELLA,SHELLP,
     +                          NGQP,MIJKL,MGQIJKL,
     +                          NXYZET,NXYZP,
...
CSSS             Write(*,"(3(1x,F15.10))") (BATCH (I), I=1, NBATCH)
             ...
             ELSE
                 ...
                 CALL    ERD__ATOM_JNT2DDS_TO_E0F0
     +
     +                        ( SHELLA,SHELLP,SHELLC,SHELLQ,
     +                          NGQP,MIJKL,MGQIJKL,
     +                          NXYZET,NXYZFT,NXYZP,NXYZQ,
