...
C  in the file COPYRIGHT.
         SUBROUTINE  ERD__E0F0_DEF_DSHIELD_BLOCKS
     +
     +               ( ZMAX,
...
C                   ZINT2DX1 = offset for (blocked) 2DX^(1) integrals (K4)
C                   ZINT2DY1 = offset for (blocked) 2DY^(1) integrals (K4)
C                   ZINT2DZ1 = offset for (blocked) 2DZ^(1) integrals (K4)
C
...
C
         IF (MEMORY) THEN
             ...
             MIJKL   = MIJ * MKL
             MGQIJKL = NGQP * MIJKL
             NINT2D  = MGQIJKL * (SHELLP+1) * (SHELLQ+2)
             NINT2D1 = MGQIJKL * (SHELLP+1) * (SHELLQ+1)
             MRSKL   = NRS * MKL
...
C
                    ...
                    ZINT2DX1 = ZINT2DZ  + NINT2D
                    ZINT2DY1 = ZINT2DX1 + NINT2D1
                    ZINT2DZ1 = ZINT2DY1 + NINT2D1
