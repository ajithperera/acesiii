...
C  in the file COPYRIGHT.
         SUBROUTINE  ERD__2D_DSHIELDXZ_PQ_INTEGRALS
     +
     +                    ( SHELLP,SHELLQ,
     +                      MIJ,MKL,NGQP,NGQEXQ,
     +                      PX,PY,PZ,QX,QY,QZ,
     ...
     +
C------------------------------------------------------------------------
C  OPERATION   : ERD__2D_DSHIELDXZ_PQ_INTEGRALS
         ...
         DO IJ = 1,MIJ
            ...
            DO KL = 1,MKL
               ...
               DO ROOT = 1, NGQP
                  ...
                  INT2DX1 (N,0,0) = INT2DX (N,0,0) * PQX

                  INT2DY1 (N,0,0) = INT2DZ (N,0,0) * QROOT

                  INT2DZ1 (N,0,0) = INT2DZ (N,0,1) + ZC *
     +                              INT2DZ (N,0,0)
               
                  DO K = 1, SHELLQ
                     K1 = K - 1
                     K2 = K + 1
                     INT2DX1 (N,0,K) = INT2DX (N,0,K)  * PQX -
     +                                 INT2DX (N,0,K1) * QKL * K

                     INT2DY1 (N,0,K) = INT2DY (N,0,K)  * QROOT

                     INT2DZ1 (N,0,K) = INT2DZ (N,0,K2) + ZC *
     +                                 INT2DZ (N,0,K)
                  ENDDO

                  DO I = 1, SHELLP
                     I1 = I - 1
                     INT2DX1 (N,I,0) = INT2DX (N,I,0)  * PQX +
     +                                 INT2DX (N,I1,0) * PIJ * I

                     INT2DY1 (N,I,0) = INT2DY (N,I,0)  * QROOT

                     INT2DZ1 (N,I,0) = INT2DZ (N,I,1)  + ZC *
     +                                 INT2DZ (N,I,0)
                  ENDDO
                  ...
                  DO I = 1, SHELLP
                     DO K = 1, SHELLQ
                        ...
                        INT2DX1 (N,I,K) = INT2DX (N,I,K)  * PQX      +
     +                                    INT2DX (N,I1,K) * PIJ * I  -
     +                                    INT2DX (N,I,K1) * QKL * K

                        INT2DY1 (N,I,K) = INT2DY (N,I,K)  * QROOT

                        INT2DZ1 (N,I,K) = INT2DZ (N,I,K2) + ZC *
     +                                    INT2DZ (N,I,K)
                     ENDDO
                  ENDDO
...
CSSS                  QROOT = RTS(N) * (1.0D0 - RTS(N)) * TWORHO
                  ...
                  INT2DX1 (N,0,0) = INT2DZ (N,0,0) * PQX
...
CSSS                  QROOT = RTS(N) * (1.0D0 - RTS(N)) * TWORHO
                  ...
                  INT2DX1 (N,0,0) =  INT2DX (N,0,0) * PQX
                  INT2DX1 (N,0,1) =  INT2DX (N,0,1) * PQX -
     +                               INT2DX (N,0,0) * QKL

                  INT2DY1 (N,0,0) =  INT2DY (N,0,0) * QROOT
                  INT2DY1 (N,0,1) =  INT2DY (N,0,1) * QROOT 

                  INT2DZ1 (N,0,0) =  INT2DZ (N,0,1) + ZC *
     +                               INT2DZ (N,0,0)
                  INT2DZ1 (N,0,1) =  INT2DZ (N,0,2) + ZC *
     +                               INT2DZ (N,0,1)
     ...
     +                                 INT2DX (N,0,K2) * QKL * K
                     ...
                     INT2DZ1 (N,0,K) = INT2DZ (N,0,K1) + ZC  *
     +                                 INT2DZ (N,0,K) 
                 ENDDO
...
CSSS                  QROOT = RTS(N) * (1.0D0 - RTS(N)) * TWORHO
                  ...
                  INT2DX1 (N,0,0) = INT2DX (N,0,0) * PQX
                  INT2DX1 (N,1,0) = INT2DX (N,1,0) * PQX + 
     +                              INT2DX (N,0,0) * PIJ

                  INT2DY1 (N,0,0) = INT2DY (N,0,0) * QROOT 
                  INT2DY1 (N,1,0) = INT2DY (N,1,0) * QROOT 

                  INT2DZ1 (N,0,0) = INT2DZ (N,0,1) +
     +                              INT2DZ (N,0,0) * ZC
                  INT2DZ1 (N,1,0) = INT2DZ (N,1,1) +
     +                              INT2DZ (N,1,0) * ZC
...
C
                 IF (SHELLQ.LE.SHELLP) THEN
                    ...
                    DO K = 1,SHELLQ
                       ...
                       INT2DX1 (N,1,K) = INT2DX (N,1,K)  * PQX +
     +                                   INT2DX (N,0,K)  * PIJ -
     +                                   INT2DX (N,1,K2) * QKL * K

                       INT2DY1 (N,1,K) = INT2DY (N,1,K)  * QROOT

                       INT2DZ1 (N,1,K) = INT2DZ (N,1,K1) +
     +                                   INT2DZ (N,1,K)  * ZC
     ...
     +                                      INT2DZ (N,I,K)  * ZC
                 ...
                 ELSE
                    ...
                    DO I = 1,SHELLP
                       ...
                       INT2DX1 (N,I,1) =  INT2DX (N,I,1)  * PQX + 
     +                                    INT2DX (N,I1,1) * PIJ * I -
     +                                    INT2DX (N,I,0)  * QKL 

                       INT2DY1 (N,I,1) =  INT2DY (N,I,1)  * QROOT 

                       INT2DZ1 (N,I,1) =  INT2DZ (N,I,2)  +
     +                                    INT2DZ (N,1,1)  * ZC
