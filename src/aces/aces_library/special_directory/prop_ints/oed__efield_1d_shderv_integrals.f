...
C  in the file COPYRIGHT.
         SUBROUTINE  OED__EFIELD_1D_SHDERV_INTEGRALS
     +
     +                    ( MGIJCEN,
     +                      SHELLA,SHELLB,
     +                      DERA,DERB,
...
C
         ...
         IF (DERA.EQ.1) THEN
             ...
             IF (SHELLA.EQ.0) THEN
                 ...
                 DO B = 0,SHELLB
                 DO N = 1,MGIJCEN
CSSS                    OUT1D (N,0,B) = EXP2A (N) * INT1D (N,1,B)
...
C                    Write(6,*) 
             ...
             ELSE IF (SHELLA.EQ.1) THEN
                 ...
                 DO N = 1,MGIJCEN
CSSS                    OUT1D (N,0,B) = EXP2A (N) * INT1D (N,1,B)
CSSS                    OUT1D (N,1,B) = EXP2A (N) * INT1D (N,2,B)
CSSS     +                                        - INT1D (N,0,B)
C
                    OUT1D (N,0,B) =  INT1D (N,1,B)
...
CSSS     +                                           - INT1D (N,0,B)
                    ...
                    DO A = 2,SHELLA
                       ...
                       AP = A + 1
                       DO N = 1,MGIJCEN
CSSS                           OUT1D (N,A,B) = EXP2A (N) * INT1D (N,AP,B)
CSSS     +                                          - F * INT1D (N,AM,B)
...
C
         IF (DERB.EQ.1) THEN
             ...
             IF (FIRST) THEN
                 ...
                 DO A = 0,SHELLA
                 DO N = 1,MGIJCEN
CSSS                    OUT1D (N,A,0) = EXP2B (N) * INT1D (N,A,1)
                    OUT1D (N,A,0) =  INT1D (N,A,1)
                 ...
                 IF (SHELLB.GT.0) THEN
                     DO A = 0,SHELLA
                     DO N = 1,MGIJCEN
CSSS                        OUT1D (N,A,1) = EXP2B (N) * INT1D (N,A,2)
CSSS     +                                            - INT1D (N,A,0)

                        OUT1D (N,A,1) = INT1D (N,A,2)
                 ...
                 IF (SHELLB.GT.1) THEN
                     ...
                     DO B = 2,SHELLB
                        ...
                        DO A = 0,SHELLA
                        DO N = 1,MGIJCEN
CSSS                           OUT1D (N,A,B) = EXP2B (N) * INT1D (N,A,BP)
CSSS     +                                           - F * INT1D (N,A,BM)
                           OUT1D (N,A,B) =  INT1D (N,A,BP)
...
CSSS                        F = F + ONE
             ...
             ELSE
                 ...
                 DO A = 0,SHELLA
                 DO N = 1,MGIJCEN
CSSS                    OUT1D (N,A,0) = OUT1D (N,A,0) +
CSSS     +                              EXP2B (N) * INT1D (N,A,1)
     ...
     +                              INT1D (N,A,1)
                 ...
                 IF (SHELLB.GT.0) THEN
                     DO A = 0,SHELLA
                     DO N = 1,MGIJCEN
CSSS                        OUT1D (N,A,1) = OUT1D (N,A,1) +
CSSS     +                                  EXP2B (N) * INT1D (N,A,2)
CSSS     +                                            - INT1D (N,A,0)
     ...
     +                                  INT1D (N,A,2)
                 ...
                 IF (SHELLB.GT.1) THEN
                     ...
                     DO B = 2,SHELLB
                        ...
                        DO A = 0,SHELLA
                        DO N = 1,MGIJCEN
CSSS                           OUT1D (N,A,B) = OUT1D (N,A,B) +
CSSS     +                                     EXP2B (N) * INT1D (N,A,BP)
CSSS     +                                           - F * INT1D (N,A,BM)
