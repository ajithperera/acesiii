...
C  in the file COPYRIGHT.
         SUBROUTINE  OED__DSHIELD_DERV_3CEN_PCGTO_BLOCK
     +
     +                    ( NBATCH,
     ...
     +
C------------------------------------------------------------------------
C  OPERATION   : OED__DSHIELD_DERV_3CEN_PCGTO_BLOCK
...
C                1D electric field integrals (only a small change
C                to nuclear attraction integrls code)
C
C
         IF (ATOMIC) THEN
             L = 0
             ...
             DO IJ = NIJBEG,NIJEND
                ...
                DO NC = 1,NCEN
                   ...
                   TVAL (N) = P * RNPCSQ
CSSS                   SCALEN = SCALEM * NCHARGE (IXC)
                   SCALEN = SCALEM 
